...
<HTML xmlns="http://www.w3.org/1999/xhtml"><HEAD><TITLE>青鸟租房 - 首页</TITLE>
  ...
  <script language="JavaScript">
    $(function(){  //加载事件
      //1.发送异步请求获取类型，进行显示
      $.post("/con3/selectAlls",null,function (data) {
      ...
      //1.发送异步请求获取区域，进行显示
      $.post("/con1/selectAlls",null,function (data) {
        for(var i=0;i<data.length;i++){
          //创建节点
          var node=$("<option value='"+data[i].id+"'>"+data[i].name+"</option>");
          //追加节点
      ...
      function loadStreet(){
        ...
        if(did!="") {
          ...
          $.post("/con2/getOneStreets", {"did": did}, function (data) {
...
<DIV id=header class=wrap>
    ...
    <DIV class=search><span style="color:red; font-size: 24px;">欢迎:${users.name}</span><LABEL class="ui-green searchs"><input
            type="button" value="用户租房管理" onclick=location.href='/con5/Uhouse'></LABEL>
        <LABEL class=ui-green><INPUT onclick='document.location="/page/login.jsp"' value="退       出" type=button name=search></LABEL>
    </DIV>
<DIV id=navbar class=wrap>
  <DL class="search clearfix">
    <FORM id=sform method=post action="/con5/PageSearch">
      <input  type="hidden" id="setPage" name="page" value="1"/>
      标题：<INPUT class=text type=text name=title value="${page.title}">
      ...
      价格:<input type="tex" name="startPrice" size="8" value="${page.startPrice}">-<input name="endPrice" type="text" size="8" value="${condition.endPrice}">
...
<DIV class="main wrap">
  <TABLE class=house-list>
    ...
    <c:forEach items="${pageInfo.list}" var="h">
      <TR>
        <TD class=house-thumb><span><A href="/con5/houseOne?id=${h.id}&$userId=${h.userId}" target="_self"><img src="http://localhost:81/${h.path}" width="100" height="75" alt=""></A></span></TD>
        <TD>
          <DL>
            <DT><A href="/con5/houseOne?id=${h.id}&$userId=${h.userId}" target="_self">${h.title}</A></DT>
